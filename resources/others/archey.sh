...
## OS version
versionNumber=$(sw_vers -productVersion) # Find version number
...
if [ ${versionMinor} -le 11 ]; then
    version="OS X $(sw_vers -productVersion) ${versionString}"
else
    version="macOS $(sw_vers -productVersion) ${versionString}"
fi
...
${ORANGE}  ########################        ${normal}CPU: ${color}$cpu${color}
